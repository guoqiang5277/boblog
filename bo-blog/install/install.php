...
if ($v=='2') {
	...
	$mysqlver=mysqli_get_server_info($con);
	if ($mysqlver<'3') {
...
if ($v=='3') {
	...
	$result=db_query($con,$setup_query);
	if (!$result) template("<div class='log'>{$lang[1]}</div><div class='mes'>{$errmsg}</div>");
	$setup_query="INSERT INTO `{$db_prefix}links` VALUES (0, 'Bo-Blog', 'http://www.bo-blog.com', '', 'Bo-Blog旧官网地址，已经停止开发', 0, 0, 1, '', ''), (1, '问题上报地址', 'https://github.com/guoqiang5277/boblog/issues', '', '问题上报地址', 0, 0, 1, '', ''), (2, '代码下载地址', 'https://github.com/guoqiang5277/boblog', '', '代码下载地址', 0, 0, 1, '', '')";
	$result=db_query($con,$setup_query);
	if (!$result) template("<div class='log'>{$lang[1]}</div><div class='mes'>{$errmsg}</div>");
